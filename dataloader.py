...
class VOLWindowDataset(Dataset):
    ...
    def create_windows(self):
        ...
        for t in range(self.window_size, N - max(self.horizons)):
            ...
            y_list.append(y_t)
        return torch.tensor(np.array(X_list), dtype=torch.float32), torch.tensor(
            np.array(y_list), dtype=torch.long
        )
...
class OFIDataset(Dataset):
    def __init__(
        self, df: pl.DataFrame, window_size=50, horizons=[1, 2, 3, 5], levels=5
    ):
        df = df.sort("TIMESTAMP")
        npdf = df.to_pandas()
        N = len(npdf)
        ...
        for t in range(1, N):
            ask_flow, bid_flow = compute_ofi(
                dict(ask_p=ask_p[t], ask_q=ask_q[t], bid_p=bid_p[t], bid_q=bid_q[t]),
                dict(
                    ask_p=ask_p[t - 1],
                    ask_q=ask_q[t - 1],
                    bid_p=bid_p[t - 1],
        ...
        self.X = torch.tensor(
            np.stack(
                [
                    X[i - window_size : i]
                    for i in range(window_size, len(X) - max(horizons))
                ],
                dtype=np.float32,
            )
        )
...
class DeepLOBDataset(Dataset):
    def __init__(self, df: pl.DataFrame, window_size=50, horizons=[1, 2, 3], levels=5):
        ...
        for t in range(window_size, N - max(horizons)):
            vol_w = np.stack(
                [bid[t - window_size : t], ask[t - window_size : t]], axis=-1
            )  # (T, levels, 2)
            ret0 = mid[t - 1]
            labels = []
            for h in horizons:
