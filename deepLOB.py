...
class DeepLOB(nn.Module):
    ...
    def __init__(self, levels=5, time_steps=50, horizons=[1]):
        ...
        self.lstm = nn.LSTM(input_size=64 * ((time_steps)), hidden_size=64, batch_first=True)
...
def train_deeplob(model, loader, optimizer, criterion, device, epochs=10):
    ...
    for ep in range(epochs):
        ...
        print(f"Epoch {ep+1}: Loss={total_loss/len(loader):.4f}, Acc={correct/total:.4f}")


if __name__ == "__main__":
    from dataloader import DeepLOBDataset
    import polars as pl
