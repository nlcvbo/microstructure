...
class DeepVOL(nn.Module):
    ...
    def forward(self, x):
        ...
        z = torch.relu(self.temp1(z)); z = torch.relu(self.temp2(z))
        z = torch.relu(self.spatial(z))
        z = torch.relu(self.temp3(z)); z = torch.relu(self.temp4(z))
        z = z.squeeze(2)  # shape (B,32,T)
        i1 = self.inc1(z); i3 = self.inc3(z)
        i5 = self.inc5(z); im = self.inc_lin(self.incmax(z))
...
if __name__ == "__main__":
    from dataloader import make_dataloader_VOL
    import polars as pl
